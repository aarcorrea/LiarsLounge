...
public class CardThrow implements ICardThrow {
    ...
    // 1.8.8 colored dust internally maps to redstone dust so its behavior is weird
    public static float[] legacyDustColor(int r, int g, int b) {
        return new float[]{
                Math.max(r / 255f, Float.MIN_NORMAL),
                g / 255f,
                b / 255f
        };
    }
    ...
    public void throwCards(Collection<Player> players, Collection<Player> spectators, Player thrower, int amount, Runnable endTask) {
        ...
        Location tableLocation = arena.getTableLocation();
    ...
    private void spawnCardForViewer(Player viewer, Location target, Player thrower) {
        ...
        viewerEntityIds.computeIfAbsent(viewer.getUniqueId(), k -> new ArrayList<>()).add(entityId);
        ...
        for (int t = 0; t < travelTicks; t++) {
            int moveTaskId = Bukkit.getScheduler().runTaskLater(instance, () -> {
                ...
                for (int i = 1; i <= trailLength; i++) {
                    ...
                    float[] color = legacyDustColor(r, g, b);

                    Location particlePos = pos.clone()
                            .subtract(delta.clone().multiply(i))
                            .add(0, 1, 0);

                    particlePos.getWorld().spigot().playEffect(
                            particlePos,
                ...
                sendTeleport(viewer, entityId, pos);
    ...
    private void startCloseForViewers(Runnable endTask) {
        ...
        closeTask = new BukkitRunnable() {
            ...
            public void run() {
                ...
                Iterator<Map.Entry<UUID, List<Integer>>> it = viewerEntityIds.entrySet().iterator();
                while (it.hasNext()) {
                    Map.Entry<UUID, List<Integer>> entry = it.next();
                    ...
                    for (int id : entry.getValue()) {
                        Location pos = entityPositions.get(id);
                        if (pos == null) continue;
                        pos.subtract(0, perTick, 0);
                        sendTeleport(viewer, id, pos);
                    }
                }
                ...
                if (tick > closeTicks) {
                    cancel();
                    for (Map.Entry<UUID, List<Integer>> entry : viewerEntityIds.entrySet()) {
                        Player viewer = Bukkit.getPlayer(entry.getKey());
    ...
    public void forceStop() {
        ...
        for (Map.Entry<UUID, List<Integer>> e : viewerEntityIds.entrySet()) {
    ...
    private void sendTeleport(Player viewer, int entityId, Location loc) {
        arena.sendDebugMsg("CardThrow.java - Teleporting for viewer " + viewer.getName());
        PacketPlayOutEntityTeleport p = new PacketPlayOutEntityTeleport(
                entityId,
                (int) Math.floor(loc.getX() * 32.0),
                (int) Math.floor(loc.getY() * 32.0),
                (int) Math.floor(loc.getZ() * 32.0),
                (byte) 0, (byte) 0, false
        );
        VersionWrapper.sendPacket(viewer, p);
    }
